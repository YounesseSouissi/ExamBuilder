...
export default function CreateExamen() {
  ...
  const onSubmit = async (values) => {
    ...
    return UserApi.generateExam(values)
  };
  const handleUpdate = async () => {
    ...
    try {
      if (ids.length > 0) {
        setIsLoading(true)
        await UserApi.updateExam(ids, chapitresIds, idsExist)
        const { data } = await UserApi.getExam()
